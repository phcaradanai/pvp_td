...
## Purpose
- Renders the prototype flow (`arsenal_preview` -> `shared_pool_preview` -> `draft_preview` -> `phase_flow_preview` -> `result_preview`).
...
- **NO REWARDS**: Does not update server inventory or apply unlocks.
